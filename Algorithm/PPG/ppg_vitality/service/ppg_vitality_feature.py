...
def map_age_vitality(age):
    if age < 14:
        return [1, 0]
    elif age >= 14:
        return [0, 1]

def encode_vitality(df):
    df_new = df.copy()
    
    # Apply age and gender mapping
    age_encoded = df_new['age'].apply(map_age_vitality).apply(pd.Series)
...
def ppg_vitality_feature(X, fs, win_min, age, gender, 
                           augmentation = 1, num_top_samples: int = 5):
    # List of HRV features and other relevant features
    hrv_features_vitality = vitality_feature()
    # Generate valid features and select relevant columns

    vitality_feat = generate_valid_feature(X, fs, win_min, augmentation, 
                                             num_top_samples)[hrv_features_vitality]
    vitality_feat['age'] = age
    vitality_feat['gender'] = gender
    
    # Encode features
    vitality_feat = encode_vitality(vitality_feat)
    
    return vitality_feat