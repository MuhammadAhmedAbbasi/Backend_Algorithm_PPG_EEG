...
# Function to encode age and gender into separate columns
def encode_depression(df):
    df_new = df.copy()
...
# Function to process PPG data and calculate depression features
def ppg_depression_feature(X, fs, win_min, age, gender, 
                           augmentation = 1, num_top_samples: int = 5):
    # List of HRV features and other relevant features
    hrv_features_depression = depession_features()
    # Generate valid features and select relevant columns

    depression_feat = generate_valid_feature(X, fs, win_min, augmentation, 
                                             num_top_samples)[hrv_features_depression]
    depression_feat['age'] = age
    depression_feat['gender'] = gender
    
    # Encode features
    depression_feat = encode_depression(depression_feat)
    
    return depression_feat
