...
class eeg_anxiety_model():
    ...
    # 预测函数
    def predict(self, fp1_fft: list, fp2_fft: list) -> bool:
        fp1_fft = np.abs(np.fft.fft(fp1_fft))[0:64].astype(np.float32)
        fp1_fft = fp1_fft / np.sum(fp1_fft)
        fp2_fft = np.abs(np.fft.fft(fp2_fft))[0:64].astype(np.float32)
